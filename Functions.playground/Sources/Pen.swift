...
open class Pen {
    ...
    open static var delay: Double = 2
    ...
    open func move(_ steps: Int) {
        ...
        moved(oldPosition, to: position)
    }
    
    open func moveTo(_ x: Int, y: Int) {
      moveTo(Double(x), y: Double(y))
    }
    
    open func moveTo(_ x: Double, y: Double) {
        let oldPosition = position
    ...
    open func setColor(_ red: Double, green: Double, blue: Double) {
        color = SKColorWithRGB(r: Int(red*255), g: Int(green*255), b: Int(blue*255))
    }
    ...
    func moved(_ from: CGPoint, to: CGPoint) {
        if penIsDown {
