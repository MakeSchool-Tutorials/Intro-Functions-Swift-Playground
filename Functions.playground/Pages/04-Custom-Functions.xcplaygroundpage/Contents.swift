...
/*:
 ...
 ところで、"関数の中身"のテキストはすべてインデントされていることに気が付きましたか？プログラミングでは、中カッコの中はインデントするのが一般的です。(Xcodeが自動的にインデントしない場合は、tabキーを押すだけ簡単にできます)お手本のインデントを真似してコードを書いてみましょう。より読みやすくするためには大切なことです。

 ### Playgroundsについて

 - important:(重要)自分の関数は、定義を作ったページでのみ使うことができます。したがって、次のページに進むと`drawSquare`関数は使えなくなってしまいます。これはSwift Playgroundの特徴なので気にすることはありません。コードをコピーする必要がある時は教えてあげますね。
