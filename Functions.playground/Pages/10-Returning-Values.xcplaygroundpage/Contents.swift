...
/*:
 ...
 - note: （メモ）円の一周は360度つまり2πラジアンです。半周は180度で、πラジアンです。四分の一の円は、90度でπ/2ラジアンです。
 */
/*:
 - note: （メモ）`Double.pi` はSwiftが持つ定数で、Double型のπを表します。
...
/*:
 ...
 - important: （重要)この関数は何があっても`ReturnType`を返さなくてはいけません！

 ## 角度からラジアンへ

 戻り値を持つ関数を定義してみましょう！

 - callout(チャレンジ): 下に`degreesToRadians`を書いて、テストコードをアンコメントしましょう。 `Double`型の`degrees`をパラメタとして受け取り、`Double`を返します。
 */
/*:
 - note: （メモ）角度からラジアンへの変換の計算は、上に書いてある関数の計算を逆にして使えますね。`180`で割って、`Double.pi` (π)をかける
...
/*:
 ...
 - note: （メモ）1/2πはだいたい1.57、π はだいたい3.14, 2πは約6.28です。書いた関数がこの数字と合っているか確かめましょう。

 ## drawPolygonを書き直す

 - callout(チャレンジ): `calculateRotationForPolygon`関数を下に作ります。`Int`型の`sides`をパラメタとして受け取り、`Double`の値を返します。前に書いた `drawPolygon`関数をコピーしてきて使いましょう。
...
//: - callout(ヒント): `sides(辺の数)`は `Int` から `Double` へ _キャスト(変換)_  する必要があります。Swiftはデータ型にとても厳しいのです！`Double(sides)`と書くことで、_キャスト(変換)_ することができます。
