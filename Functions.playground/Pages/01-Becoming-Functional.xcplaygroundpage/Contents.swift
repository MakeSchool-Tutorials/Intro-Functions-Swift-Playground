...
/*:
 ...
 A _function call_ looks a lot like a variable but it has parenthesis after it. Function calls take the form of:
 ...
 It's important to keep in mind that functions (like variables) must be defined before they are called. We have already defined the `rotateNinety` function for you in another file and we'll be This means you cannot just call `moveThirty()` and expect it to work. A `moveThirty` has not been defined yet. We'll show you how to define your own functions soon but let's focus on calling them for now.
