...
/*:
 ...
 Where `func` is the keyword used to define a function, followed by the function name, an empty pair of parenthesis, and a pair of curly braces (`{` `}`) surrounding the code contained in the function.
 ...
 If you compare this to the earlier functions we defined, you'll see the difference is the parameter name and type inside of the parenthesis. It starts off with the same keyword `func`, followed by the function name, parenthesis and curly braces. Remember, anything "inside" the curly braces gets indented once (one press of the tab key if Xcode doesn't do it for you automatically).
 ...
 Remember, `sideLength` becomes a constant inside of the functions body. You can directly use constants and variables as parameters when you call other functions! Test out your new method with a function call of `drawSquare(150)`.
