...
/*:
 ...
 - callout(コードを設計しよう):紙とペンを使って多角形を描いてみましょう。`move`関数や`rotate`関数をどのように使えばうまく描けるでしょうか？ステップをひとつずつ書き出し、それを声に出して読み返しながら擬似コードを手書きで「実行」してみましょう。うまくできましたか？
 ...
 */
/*:
  - callout(チャレンジ):それぞれの多角形の擬似コードができたところで、これらをSwiftコードに書き換える作業にうつります。 `drawTriangle`, `drawPentagon`,`drawHexagon` の3つの関数を定義して、それぞれ試してみましょう！
